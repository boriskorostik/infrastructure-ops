...
# ===== IP адреса =====
/ip address
add address=10.17.16.1/20 interface=bridge network=10.17.16.0

/ip dns static
add address=10.17.16.1 comment=defconf name=router.lan

# ===== DHCP =====
/ip pool
add name=dhcp_pool ranges=10.17.31.2-10.17.31.254

/ip dhcp-server
add name=dhcp1 interface=bridge address-pool=dhcp_pool disabled=no

/ip dhcp-server network
add address=10.17.16.0/20 gateway=10.17.16.1 dns-server=77.88.8.8,8.8.8.8
...
add action=netmap chain=dstnat comment=ssh dst-port=22100 in-interface=ether1 \
    protocol=tcp to-addresses=10.17.16.10 to-ports=22100
add action=netmap chain=dstnat comment=inbuilding dst-port=29107 \
    in-interface=ether1 protocol=tcp to-addresses=10.17.16.10 to-ports=29107
add action=netmap chain=dstnat comment=inbuilding dst-port=19090 \
    in-interface=ether1 protocol=udp to-addresses=10.17.16.10 to-ports=19090
add action=netmap chain=dstnat comment=video dst-port=5000-5400 \
    in-interface=ether1 protocol=udp to-addresses=10.17.16.10 to-ports=5000-5400
add action=netmap chain=dstnat comment=video dst-port=5000-5100 \
    in-interface=ether1 protocol=tcp to-addresses=10.17.16.10 to-ports=5000-5100
...
add action=netmap chain=dstnat comment=askue dst-port=14200 \
    in-interface-list=WAN log=yes protocol=tcp to-addresses=10.17.16.134 \
    to-ports=14200
...
add action=netmap chain=dstnat comment=zbx in-interface-list=WAN protocol=icmp \
    src-address-list=admin to-addresses=10.17.16.10
...
add action=jump chain=output content="invalid user name or password" \
    dst-address-list=!admin jump-target=FB-WB protocol=tcp src-port=9657
